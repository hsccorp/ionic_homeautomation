...
.controller('LoginCtrl', ['$scope', '$rootScope', 'zm', '$ionicModal', 'ZMDataModel', '$ionicSideMenuDelegate', '$ionicPopup', '$http', '$q', '$ionicLoading', 'zmAutoLogin', '$cordovaPinDialog', 'EventServer', '$ionicHistory', '$state', '$ionicActionSheet', 'SecuredPopups', '$localstorage', '$stateParams',
	function ($scope, $rootScope, zm, $ionicModal, ZMDataModel, $ionicSideMenuDelegate, $ionicPopup, $http, $q, $ionicLoading, zmAutoLogin, $cordovaPinDialog, EventServer, $ionicHistory, $state, $ionicActionSheet, SecuredPopups, $localstorage, $stateParams) {
	...
	$scope.callLogin = function(){
        ...
        console.log($scope.loginData.username,"::login auth:::::",$scope.loginData.password);
	...
	var url = 'http://192.168.1.215/zm/index.php';
	var urlDemoNinja = 'https://demo.zoneminder.com/zm/index.php';

			$http.post(urlDemoNinja, demoData, config).then(function (res)
            {
				//in success block
        ...
        /*$http({
             method: 'POST',
                //withCredentials: true,
                url: 'http://192.168.1.215/zm/index.php',
                transformRequest: function (obj) {
                    var str = [];
                    for (var p in obj)
                        str.push(encodeURIComponent(p) + "=" +
    ...
    $scope.getEventsList = function(){
        var config = {
            ...
            var url = 'https://demo.zoneminder.com/zm/api/events/228381.json';
		...
		$http.get('https://demo.zoneminder.com/zm/api/events.json' /*,{timeout:15000}*/ )
                .success(function (data) {
